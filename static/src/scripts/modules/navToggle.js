class NavToggle {
    constructor(navBtnId = "") {
        ...
        this.mainNavParents = this.mainNav ? this.mainNav.querySelectorAll("ul > li > a.parent") : null
    ...
    events() {
        ...
        this.mainNavParents.forEach(parent => parent.nextElementSibling.style.height = 0 + 'px')

        this.mainNavParents.forEach(parent=> parent.addEventListener('click', this.handleParentClick.bind(this)))
        ...
        window.addEventListener('resize', this.debounce(this.handleResize.bind(this)).bind(this))
    ...
    handleClick(e) {
        ...
        this.mainNav.style.height = this.mainNav.classList.contains('active') ? (this.mainNav.scrollHeight + childHeight) + 'px' : 0 + 'px'
        // close children
    ...
    handleMouseIn(e) {
        this.paths = !this.paths ? e.target.getElementsByTagName('path') : this.paths
    ...
    handleMouseOut(e) {
        this.paths = !this.paths ? e.target.getElementsByTagName('path') : this.paths
        for (const path of this.paths) { 
            path.classList.remove(path.classList.contains("active") ? "active" : null) 
        }
